...
const AttendanceSystem = () => {
  ...
  const [adminTab, setAdminTab] = useState('employees'); // employees, calendar, tasks, cleaning, attendance
  const [newCleaningTask, setNewCleaningTask] = useState({ pagi: '', malam: '' }); // For admin to add new cleaning tasks
  ...
  const distributeTasks = (shift, employeeCount) => {
    const tasks = shiftTasks[shift];
    const today = new Date();
    
    // Calculate rotation offset based on:
    // 1. Day of year (ensures daily rotation)
    // 2. Shift type (pagi vs malam get different distributions)
    const dayOfYear = Math.floor((today - new Date(today.getFullYear(), 0, 0)) / 86400000);
    const shiftOffset = shift === 'pagi' ? 0 : 1;
    
    // Rotation index: changes every day and differs between shifts
    // This ensures fair rotation over time
    const rotationIndex = (dayOfYear + shiftOffset) % employeeCount;
    
    // Create seed that combines date and shift for consistent but varied distribution
    const seed = today.getFullYear() * 10000 + (today.getMonth() + 1) * 100 + today.getDate() + shiftOffset;
    
    // Shuffle tasks based on today's date and shift
    const shuffledTasks = shuffleWithSeed(tasks, seed);
    
    // Distribute tasks evenly
    const tasksPerEmployee = Math.floor(shuffledTasks.length / employeeCount);
    const distributedTasks = [];
    
    // Rotate the starting point for fair distribution over time
    for (let i = 0; i < employeeCount; i++) {
      // Apply rotation to ensure different employees get different task sets each day
      const rotatedIndex = (i + rotationIndex) % employeeCount;
      const startIdx = rotatedIndex * tasksPerEmployee;
      const endIdx = startIdx + tasksPerEmployee;
      distributedTasks[i] = shuffledTasks.slice(startIdx, endIdx);
    }
    
    // Distribute remaining tasks with rotation
    const remainingTasks = shuffledTasks.slice(employeeCount * tasksPerEmployee);
    remainingTasks.forEach((task, idx) => {
      const targetEmployee = (idx + rotationIndex) % employeeCount;
      distributedTasks[targetEmployee].push(task);
    });
  ...
  const initializeEmployees = () => {
    return [
      { 
        ...
        checkInTime: null, cleaningTasks: [], workTasks: [], shift: null, 
  ...
  const checkInEmployee = async (empId) => {
    ...
    console.log(`✅ CHECK IN EMPLOYEE: empId=${empId}`);
    ...
    const updated = employees.map((emp, empIndex) => {
        if (emp.id === empId) {
          ...
          // Get distributed tasks for this employee
          // Get all employees (total count for fair distribution)
          const totalEmployees = employees.length;
          const distributedTasks = distributeTasks(currentShift, totalEmployees);
          
          // Use employee index (0, 1, 2) for consistent task assignment
          const employeeTaskIndex = empIndex; // empIndex from map function
          const employeeTasks = distributedTasks[employeeTaskIndex] || [];
          ...
          return {
            ...
            cleaningTasks: alreadyInShift ? emp.cleaningTasks : employeeTasks.map((task, i) => ({
              id: Date.now() + emp.id * 1000 + i,
              task: task,
              completed: false,
              priority: 'normal',
              progress: 0,
    ...
    const checkedIn = updated.filter(e => e.checkedIn);
    setCheckedInEmployees(checkedIn);
    
    // Update React state
    setEmployees(updated);
    
    // Update yearlyAttendance calendar with check-in status  
    ...
    if (emp) {
      ...
      // Only update calendar if NOT overtime (keep first shift status)
      if (!isOvertime) {
        // Determine status based on check-in time
        let calendarStatus = 'hadir';
        let lateHours = 0;
        let maxCheckInHour = shiftConfig.maxCheckIn;
    ...
    // ✅ CRITICAL FIX: Save fresh data directly
    await saveEmployeesDirectly(updated);
  ...
  const startBreak = async (empId) => {
    ...
    console.log(`☕ START BREAK: empId=${empId}`);
    ...
    console.log(`✅ Start break completed`);
    setIsProcessing(false);
    isProcessingRef.current = false;
  };
  ...
  const endBreak = async (empId) => {
    ...
    console.log(`✅ END BREAK: empId=${empId}`);
    ...
    console.log(`✅ End break completed`);
    setIsProcessing(false);
    isProcessingRef.current = false;
  };
  ...
  const endShift = async (empId) => {
    ...
    console.log(`🏁 END SHIFT called: empId=${empId}`);
    ...
    const updatedEmployees = employees.map(emp => {
      if (emp.id === empId && emp.checkedIn) {
        // ✅ CRITICAL FIX: Save all tasks (completed AND incomplete) to history before clearing
        const allCleaningTasks = (emp.cleaningTasks || []).map(t => ({
          ...t,
          taskType: 'cleaning',
        ...
        console.log(`📊 ${emp.name} end shift: ${allTasks.length} tasks archived (shiftDate: ${now.toDateString()})`);
    ...
    console.log(`✅ State updated, saving to Firebase...`);
    addNotification(`🏁 ${foundEmp?.name} selesai shift (${timeStr})`, 'success');
    
    // ✅ CRITICAL FIX: Save the UPDATED array directly (not from stale state)
    try {
      await dbService.saveEmployees(updatedEmployees);
      console.log('✅ Direct save to Firebase completed');
    } catch (error) {
  ...
  const confirmIzin = async () => {
    ...
    console.log(`📋 CONFIRM IZIN: ${izinReason}`);
    ...
    console.log(`✅ Confirm izin completed`);
    setIsProcessing(false);
    isProcessingRef.current = false;
  };
  ...
  const endIzin = async (empId) => {
    ...
    console.log(`✅ END IZIN: empId=${empId}`);
    ...
    console.log(`✅ End izin completed`);
    setIsProcessing(false);
    isProcessingRef.current = false;
  };
                ...
                if (initialData) {
                  ...
                  if (initialData.employees) {
                    ...
                    setEmployees(prevEmployees => {
                      // If no previous employees (first load), use Firebase data
                      if (!prevEmployees || prevEmployees.length === 0) {
                        ...
                        return initialData.employees.map(emp => ({
                          ...
                          completedTasksHistory: (emp.completedTasksHistory || []).map(task => {
                            // If task doesn't have shiftDate, try to infer from completedAt or endShiftTime
                            if (!task.shiftDate) {
                              if (task.completedAt) {
                                try {
                      ...
                      // Smart merge: preserve local completedTasksHistory if more recent
                      return initialData.employees.map(firebaseEmp => {
                        ...
                        return {
                          ...firebaseEmp,
                          completedTasksHistory: (firebaseEmp.completedTasksHistory || []).map(task => {
                            // ✅ MIGRATION: Fix old tasks missing shiftDate
                            if (!task.shiftDate && task.completedAt) {
                              try {
                                task.shiftDate = new Date(task.completedAt).toDateString();
  ...
  useEffect(() => {
              ...
              if (localEmp) {
                ...
                if (localIsCheckedIn && firebaseIsCheckedIn) {
                  ...
                  const mergeTaskCompletions = (localTasks, firebaseTasks, completedHistory) => {
                    ...
                    if (filteredFirebaseTasks.length === 0) return localTasks;
                  ...
                  const mergedCleaningTasks = mergeTaskCompletions(localEmp.cleaningTasks, firebaseEmp.cleaningTasks, localEmp.completedTasksHistory);
                  ...
                  return {
                    ...
                    cleaningTasks: mergedCleaningTasks,
                    workTasks: mergedWorkTasks,
                    completedTasksHistory: mergedHistory, // ✅ Merge history from both sources
                    // Only update non-critical fields from Firebase
                    baseSalary: firebaseEmp.baseSalary || localEmp.baseSalary,
                    isAdmin: firebaseEmp.isAdmin !== undefined ? firebaseEmp.isAdmin : localEmp.isAdmin
                  };
                }
                ...
                // Case 4: Both NOT checked in - do smart merge with task completion
                  ...
                  const hasFirebaseCleaningTasks = firebaseEmp.cleaningTasks && firebaseEmp.cleaningTasks.length > 0;
                  ...
                  const mergeTaskCompletions = (localTasks, firebaseTasks) => {
                    ...
                    if (!firebaseTasks || firebaseTasks.length === 0) return localTasks;
                  ...
                  return {
                    ...firebaseEmp,
                    // ✅ Merge task completions (prevent data loss)
                    cleaningTasks: hasFirebaseCleaningTasks && hasLocalCleaningTasks
                      ? mergeTaskCompletions(localEmp.cleaningTasks, firebaseEmp.cleaningTasks)
                      : (hasFirebaseCleaningTasks ? firebaseEmp.cleaningTasks : localEmp.cleaningTasks || []),
                    workTasks: hasFirebaseWorkTasks && hasLocalWorkTasks
                      ? mergeTaskCompletions(localEmp.workTasks, firebaseEmp.workTasks)
                      : (hasFirebaseWorkTasks ? firebaseEmp.workTasks : localEmp.workTasks || []),
    ...
    // Listen to shift tasks changes
    const unsubscribeShiftTasks = dbService.onShiftTasksChange((data) => {
      if (data && typeof data === 'object') {
        console.log('📥 RECEIVED FROM FIREBASE - SHIFT TASKS');
  ...
  const saveImmediately = async () => {
    ...
    try {
      ...
      await Promise.all([
        dbService.saveEmployees(employees),
  ...
  const migrateCompletedTasksToHistory = () => {
    ...
    setEmployees(prev => {
      const updated = prev.map(emp => {
        // Find completed tasks in current lists and add taskType marker
        const completedCleaning = emp.cleaningTasks.filter(t => t.completed).map(t => ({
          ...t,
          taskType: 'cleaning' // ✅ Add taskType marker
        }));
        ...
        const completedInCurrent = [...completedCleaning, ...completedWork];
        ...
        if (completedInCurrent.length > 0 || fixedHistory.length !== emp.completedTasksHistory.length) {
          ...
          return {
            ...
            cleaningTasks: emp.cleaningTasks.filter(t => !t.completed),
  ...
  useEffect(() => {
    const checkTasks = () => {
      ...
      setEmployees(prev => {
        prev.forEach(emp => {
          [...emp.cleaningTasks, ...emp.workTasks].forEach(task => {
  ...
  const priorityColors = {
    ...
    normal: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-700', badge: 'bg-blue-500' },
    low: { bg: 'bg-slate-50', border: 'border-slate-200', text: 'text-slate-700', badge: 'bg-slate-500' }
  };

  const statusConfig = {
    hadir: { bg: 'bg-emerald-500', label: 'Hadir' },
    libur: { bg: 'bg-blue-500', label: 'Libur' },
    lembur: { bg: 'bg-amber-500', label: 'Lembur' },
    izin: { bg: 'bg-violet-500', label: 'Izin' },
    telat: { bg: 'bg-rose-500', label: 'Telat' }
  };
  ...
  const toggleTask = async (empId, type, taskId) => {
    ...
    console.log(`☑️ TOGGLE TASK called: empId=${empId}, type=${type}, taskId=${taskId}`);
        ...
        return {
          ...
          [list]: e[list].map(t => {
            if (t.id === taskId) {
              ...
              // Track productivity when completing task
              if (nowCompleted) {
                trackProductivity(empId, type);
                addNotification(`✅ ${e.name} menyelesaikan task: ${t.task}`, 'success');
              }
              // ✅ FIX BUG #2: Add completedAtDisplay for better history display
              ...
              return { 
                ...
                completedAtDisplay: completedTime // Add display field
              };
            }
            return t;
          })
    ...
    // ✅ CRITICAL FIX: Wait for save to complete before enabling buttons
  ...
  const updateProgress = async (empId, type, taskId, progress) => {
    // No double-click protection for progress slider
  ...
  const startTask = async (empId, type, taskId) => {
    ...
    console.log(`▶️ START TASK called: empId=${empId}, type=${type}, taskId=${taskId}`);
    ...
    const empName = updatedEmployees.find(e => e.id === empId)?.name;
    console.log(`✅ ${empName} task started at ${timeStr}`);
    
    // Update React state
    setEmployees(updatedEmployees);
    
    addNotification(`▶️ ${empName} mulai task`, 'info');
  ...
  const confirmPause = async () => {
    ...
    console.log(`⏸️ CONFIRM PAUSE: ${pauseReason}`);
  ...
  const resumeTask = async (empId, type, taskId) => {
    ...
    console.log(`▶️ RESUME TASK: empId=${empId}, type=${type}, taskId=${taskId}`);
    ...
    addNotification(`▶️ ${empName} melanjutkan task`, 'info');
  ...
  const endTask = async (empId, type, taskId) => {
    ...
    const updatedEmployees = employees.map(e => {
      if (e.id === empId) {
        ...
        console.log(`🔍 Searching task in ${list}: found=${!!task}, totalTasks=${e[list]?.length}, taskId=${taskId}`);
        
        if (task) {
          taskFound = true;
          console.log(`✅ Task found: ${task.task}, startTime=${task.startTime}`);
    ...
    console.log(`💾 Saving task to Firebase...`);
  ...
  const handleDeleteTask = (empId, taskType, taskId) => {
  ...
  // Cleaning Tasks Management Functions
  const handleAddCleaningTask = (shift) => {
    const taskText = newCleaningTask[shift].trim();
    if (!taskText) {
    ...
    setShiftTasks(prev => ({
      ...prev,
      [shift]: [...prev[shift], taskText]
    }));

    setNewCleaningTask(prev => ({ ...prev, [shift]: '' }));
    addNotification(`✅ Task kebersihan shift ${shift} berhasil ditambahkan!`, 'success');
  };

  const handleDeleteCleaningTask = (shift, index) => {
    if (!window.confirm('Yakin ingin menghapus task kebersihan ini?')) return;
    ...
    addNotification('🗑️ Task kebersihan berhasil dihapus!', 'warning');
  };
  ...
  const handleResetEmployee = (empId) => {
  ...
  const deleteTask = (empId, type, taskId) => {
    setEmployees(prev => prev.map(e => e.id === empId ? { ...e, [type === 'cleaning' ? 'cleaningTasks' : 'workTasks']: e[type === 'cleaning' ? 'cleaningTasks' : 'workTasks'].filter(t => t.id !== taskId) } : e));
  };

  const addTask = (empId, type) => {
    const text = newTask[`${empId}-${type}`];
    if (!text?.trim()) return;
    setEmployees(prev => prev.map(e => {
      if (e.id === empId) {
        const list = type === 'cleaning' ? 'cleaningTasks' : 'workTasks';
        return { ...e, [list]: [...e[list], { id: e[list].length + 1, task: text, completed: false, priority: 'normal', progress: 0, startTime: null, endTime: null, duration: null, createdAt: new Date().toLocaleString('id-ID'), completedAt: null }] };
      }
      return e;
    }));
  ...
  const updatePriority = (empId, type, taskId, priority) => {
    setEmployees(prev => prev.map(e => {
      if (e.id === empId) {
        const list = type === 'cleaning' ? 'cleaningTasks' : 'workTasks';
  ...
  const calculateMonthlyStats = (name, month) => {
    ...
    const stats = { hadir: 0, libur: 0, lembur: 0, izin: 0, telat: 0, totalLateHours: 0 };
  ...
  const addAttention = () => {
    ...
    setAttentions(prev => [{ 
      ...
      completed: false, 
      createdAt: new Date().toLocaleString('id-ID'),
      readBy: [] // Array untuk track siapa yang sudah baca
    }, ...prev]);
    setNewAttentionText('');
    setNewAttentionImage(null);
  };

  const toggleAttentionRead = (attentionId, employeeName) => {
    setAttentions(prev => prev.map(att => {
      if (att.id === attentionId) {
        ...
        return {
          ...
          completed: allRead // Auto complete when all read
        };
      }
      return att;
    }));
  ...
  const toggleAttention = (id) => setAttentions(prev => prev.filter(a => a.id !== id));
  const deleteAttention = (id) => setAttentions(prev => prev.filter(a => a.id !== id));
  ...
  const AttentionSection = useMemo(() => {
    ...
    if (!showAttentionSection && active.length === 0) {
      return (
        <div className="mb-8">
          <button onClick={() => setShowAttentionSection(true)} className="flex items-center gap-2 bg-gradient-to-r from-red-500 to-orange-500 text-white px-5 py-2.5 rounded-xl hover:from-red-600 hover:to-orange-600 transition-all shadow-md text-sm font-medium">
            <Bell size={16} />
      ...
      <div className={`${currentTheme.card} rounded-2xl shadow-md border-2 p-6 mb-8`}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <span className="bg-red-500 text-white px-2.5 py-1 rounded-lg text-xs font-semibold">{active.length} Active</span>
            {active.length === 0 && (
              <button onClick={() => setShowAttentionSection(false)} className="text-slate-400 hover:text-slate-600 p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
                <X size={16} />
              </button>
            )}
          </div>
        ...
        <div className="mb-5 p-4 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl border-2 border-orange-200">
          ...
          <div className="flex items-center gap-2 mt-3">
            <label className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-300 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors text-sm">
              <Upload size={16} className="text-slate-600" />
              <span className="text-slate-600">Gambar</span>
              <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
            </label>
            {newAttentionImage && (
              <div className="relative">
                <img src={newAttentionImage} alt="Preview" className="h-10 w-10 object-cover rounded-lg border border-slate-300" />
                <button onClick={() => setNewAttentionImage(null)} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-0.5 hover:bg-red-600">
                  <X size={10} />
                </button>
              </div>
            )}
            <button onClick={addAttention} className="ml-auto flex items-center gap-1.5 bg-gradient-to-r from-red-500 to-orange-500 text-white px-4 py-2 rounded-lg hover:from-red-600 hover:to-orange-600 transition-all shadow-sm text-sm font-medium">
              <Plus size={16} />
              Tambah
            </button>
          </div>
        ...
        {active.length > 0 && (
          <div className="space-y-2.5">
            {active.map(att => {
              ...
              return (
                <div key={att.id} className="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-xl p-3.5 hover:shadow-sm transition-shadow">
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-slate-800 text-sm font-medium mb-1.5 break-words">{att.text}</p>
                      {att.image && <img src={att.image} alt="Attention" className="max-w-sm rounded-lg border border-slate-300 mb-2" />}
                      <p className="text-xs text-slate-500 mb-3">📝 {att.createdAt}</p>
                      
                      {/* Read confirmation per employee */}
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-xs font-semibold text-slate-700">Konfirmasi Baca:</p>
                    ...
                    <button onClick={() => deleteAttention(att.id)} className="text-red-500 hover:text-red-700 p-1 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-2">
              {attentions.filter(a => a.completed).map(att => (
                <div key={att.id} className={`${currentTheme.card} border-2 border-emerald-200 rounded-xl p-3 opacity-75`}>
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {att.image && <img src={att.image} alt="Attention" className="max-w-xs rounded-lg border border-slate-300 mb-2 mt-2" />}
                      <p className={`text-xs ${currentTheme.subtext}`}>
                        📝 {att.createdAt} • ✅ Selesai dibaca semua
                      </p>
                    </div>
                    <button onClick={() => deleteAttention(att.id)} className="text-slate-400 hover:text-red-500 p-1 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
  ...
  // ✅ REFACTORED: DashboardPage extracted to src/components/Dashboard/DashboardPage.jsx
  // Old inline definition removed (435 lines) - now using imported component

  // ✅ REFACTORED: ShiftSchedulePage extracted to src/components/ShiftSchedule/ShiftSchedulePage.jsx
  // All shift logic (generation, calculation, libur management) is now in the component
    ...
    const empStats = employees.map(e => {
      ...
      if (leaderboardTaskType === 'all') {
        ...
        tasksByType = [...e.cleaningTasks, ...(e.completedTasksHistory || []).filter(t => 
          // Include if from history with cleaning marker, or if no history marker (old data)
          !t.taskType || t.taskType === 'cleaning'
        )];
      } else if (leaderboardTaskType === 'work') {
        tasksByType = [...e.workTasks, ...(e.completedTasksHistory || []).filter(t => 
          // Include if from history with work marker, or if no history marker (old data)
      ...
      if (leaderboardPeriod === 'today') {
        ...
        // For total, only show completed tasks
        filtered = all.filter(t => t.completed);
      }
      
      const completed = filtered; // Already filtered for completed tasks
          ...
          if (t.pauseHistory && t.pauseHistory.length > 0) {
            const pauseTime = t.pauseHistory.reduce((pauseSum, pause) => {
              if (pause.startTime && pause.endTime) {
                const [sh, sm] = pause.startTime.split(':').map(Number);
                const [eh, em] = pause.endTime.split(':').map(Number);
                const pauseMinutes = (eh * 60 + em) - (sh * 60 + sm);
                return pauseSum + (pauseMinutes > 0 ? pauseMinutes : 0);
              }
              return pauseSum;
            }, 0);
            min = Math.max(0, min - pauseTime); // Subtract pause time, minimum 0
          }
      ...
      return { 
        ...
        completedTasks: completed, // Store completed tasks for detail view
        rate: filtered.length > 0 ? ((completed.length / filtered.length) * 100).toFixed(1) : 0, 
        avgMin, 
        withTime: withDur.length,
        totalAll,
        completedAll,
        cleaningCount: e.cleaningTasks.filter(t => t.completed && (leaderboardPeriod === 'total' || (t.completedAt && new Date(t.completedAt) >= (leaderboardPeriod === 'today' ? new Date().setHours(0,0,0,0) : leaderboardPeriod === 'week' ? weekStartDate : monthStartDate)))).length,
        workCount: e.workTasks.filter(t => t.completed && (leaderboardPeriod === 'total' || (t.completedAt && new Date(t.completedAt) >= (leaderboardPeriod === 'today' ? new Date().setHours(0,0,0,0) : leaderboardPeriod === 'week' ? weekStartDate : monthStartDate)))).length
      };
    });

    const taskLeaders = [...empStats].sort((a, b) => b.completed - a.completed);
    const speedLeaders = [...empStats].filter(e => e.withTime > 0).sort((a, b) => a.avgMin - b.avgMin);

    const getMedal = (rank) => {
      return { emoji: rank === 0 ? '🥇' : rank === 1 ? '🥈' : rank === 2 ? '🥉' : '👤' };
    };
        ...
        <div className={`${currentTheme.card} rounded-2xl shadow-md border-2 p-6`}>
          <div className="mb-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className={`text-2xl font-bold ${currentTheme.text} flex items-center gap-2`}>
                  <Trophy className="text-yellow-500" size={24} />
                  Leaderboard
                </h2>
                <p className={`text-sm ${currentTheme.subtext} mt-1`}>Performa terbaik berdasarkan task completion</p>
              </div>
              <div className="flex gap-3">
                ...
                <div className="flex gap-2 border-l pl-3">
                  {[
                    ...
                    <button
                      key={type.value}
                      onClick={() => setLeaderboardTaskType(type.value)}
                      className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                        leaderboardTaskType === type.value
          ...
          <div className="grid grid-cols-2 gap-6">
            {[
              { data: taskLeaders, title: 'Top Task Completion', icon: CheckCircle, color: 'blue', desc: 'Jumlah task selesai' },
              { data: speedLeaders, title: 'Fastest Completion', icon: Clock, color: 'emerald', desc: 'Rata-rata waktu tercepat' }
            ].map(({ data, title, icon: Icon, color, desc }) => (
              <div key={title}>
                <div className="flex items-center gap-2.5 mb-5">
                  <div className={`bg-gradient-to-r from-${color}-500 to-${color}-600 p-2.5 rounded-xl`}>
                    <Icon className="text-white" size={20} />
                  </div>
                  <div>
                    <h3 className={`text-lg font-semibold ${currentTheme.text}`}>{title}</h3>
                    <p className={`text-xs ${currentTheme.subtext}`}>{desc}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  {data.map((e, i) => {
                    ...
                    const timeStr = title.includes('Fastest') ? (e.avgMin >= 60 ? `${Math.floor(e.avgMin / 60)}j ${e.avgMin % 60}m` : `${e.avgMin}m`) : null;
                    const isExpanded = expandedLeaderboard === e.id;
                    return (
                      <div key={e.id} className={`${currentTheme.card} rounded-xl p-4 shadow-md border-2 transition-all hover:shadow-lg ${isExpanded ? 'scale-102' : 'hover:scale-102'}`}>
                        <div 
                          className="flex items-center justify-between cursor-pointer"
                          onClick={() => setExpandedLeaderboard(isExpanded ? null : e.id)}
                        >
                          <div className="flex items-center gap-3">
                            <div className="text-3xl">{medal.emoji}</div>
                            <div>
                              <h4 className={`text-base font-semibold ${currentTheme.text}`}>{e.name}</h4>
                              <p className={`text-xs ${currentTheme.subtext}`}>
                                EMP-{e.id.toString().padStart(3, '0')}
                                {leaderboardTaskType === 'all' && (
                                  <span className="ml-2">🧹 {e.cleaningCount} | 💼 {e.workCount}</span>
                                )}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
                            <div className={`text-2xl font-bold ${currentTheme.text}`}>{timeStr || e.completed}</div>
                            <div className={`text-xs ${currentTheme.subtext}`}>{timeStr ? `${e.withTime} task` : `dari ${e.total} task`}</div>
                            {!timeStr && <div className={`text-sm font-semibold mt-0.5 ${currentTheme.text}`}>{e.rate}%</div>}
                          </div>
                        </div>
                        <div className="mt-2.5 bg-slate-200 rounded-full h-1.5 overflow-hidden">
                          <div className="bg-slate-600 h-full rounded-full transition-all" style={{ width: timeStr ? '100%' : `${e.rate}%` }}></div>
                        </div>
                        ...
                        {isExpanded && (
                          <div className="mt-3 pt-3 border-t border-slate-300">
                            <div className="flex items-center justify-between mb-2">
                              <p className={`text-xs font-semibold ${currentTheme.text}`}>📋 Task yang Dikerjakan:</p>
                              <p className={`text-xs ${currentTheme.subtext}`}>{e.completedTasks?.length || 0} task</p>
                            </div>
                            {e.completedTasks && e.completedTasks.length > 0 ? (
                              <div className="space-y-1.5">
                                {/* Show first 3 tasks */}
                                {e.completedTasks.slice(0, 3).map((task, idx) => (
                                  <div key={idx} className={`${currentTheme.card} rounded-lg p-2.5 text-xs border-2 shadow-sm`}>
                                    <div className="flex items-start justify-between gap-2">
                                      <span className={`font-medium ${currentTheme.text}`}>{task.task}</span>
                                      {task.duration && <span className={`text-xs ${currentTheme.subtext}`}>{task.duration}</span>}
                                    </div>
                                    {task.completedAt && (
                                ...
                                {/* Show "more" button if more than 3 tasks */}
                                {e.completedTasks.length > 3 && (
                                  <button
                                    onClick={() => {
                                      alert(`Semua Task ${e.name}:\n\n${e.completedTasks.map((t, i) => `${i+1}. ${t.task} ${t.duration ? `(${t.duration})` : ''}`).join('\n')}`);
                                    }}
                                    className={`w-full py-2 ${currentTheme.card} hover:shadow-md rounded-lg text-xs font-medium transition-all border-2 ${currentTheme.text}`}
                                  >
                                    +{e.completedTasks.length - 3} task lainnya
                                  </button>
                                )}
                              </div>
                            ) : (
                              <p className={`text-xs ${currentTheme.subtext} italic`}>Belum ada task selesai</p>
                            )}
                          </div>
                        )}
                      </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            {[
              { label: 'Total Task', value: empStats.reduce((s, e) => s + e.total, 0) },
              { label: 'Task Selesai', value: empStats.reduce((s, e) => s + e.completed, 0) },
              { label: 'Avg Time', value: (() => {
                const valid = empStats.filter(e => e.withTime > 0);
                if (!valid.length) return '0m';
                const avg = Math.round(valid.reduce((s, e) => s + e.avgMin, 0) / valid.length);
                return avg >= 60 ? `${Math.floor(avg / 60)}j ${avg % 60}m` : `${avg}m`;
              })() }
            ].map(({ label, value }) => (
              <div key={label} className={`${currentTheme.card} p-4 rounded-xl border-2 shadow-md`}>
                <div className={`${currentTheme.subtext} text-xs font-semibold mb-1.5`}>{label}</div>
  ...
  // 📦 ORDERAN PAGE - REFACTORED TO EXTERNAL COMPONENT
  // OrderanPage component is now in: src/components/Orderan/OrderanPage.jsx
  // It uses ordersHook for all business logic - see render section below for usage

  // ✅ REFACTORED: AdminPanel extracted to src/components/Admin/AdminPanel.jsx
        ...
        <div className="max-w-md w-full">
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl border-2 border-white/20 p-8">
            ...
            <div className="text-center mb-8">
              <h1 className="text-3xl font-bold text-white mb-2">🔒 Aplikasi Terkunci</h1>
            ...
            <button
              ...
              className={`w-full px-6 py-3 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500 text-white font-medium hover:from-blue-600 hover:to-purple-600 transition-all shadow-lg ${isUnlocking ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {isUnlocking ? '⏳ Membuka...' : '🔓 Buka Aplikasi'}
            </button>

            {/* Info */}
            <div className="mt-6 text-center">
              <p className="text-xs text-slate-400">
                🔐 Aplikasi ini dilindungi untuk mencegah akses tidak sah
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${currentTheme.bg} p-6`}>
      <div className="max-w-7xl mx-auto">
        <nav className={`${currentTheme.nav} rounded-2xl shadow-md border-2 p-2 mb-8`}>
          <div className="flex items-center justify-between">
            <div className="flex gap-2">
              {[
                ...
                <button key={page} onClick={() => setActivePage(page)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-all text-sm ${activePage === page ? currentTheme.navActive : currentTheme.navInactive}`}>
                  <Icon size={16} />
                  {label}
                </button>
              ))}
            ...
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-md text-xs font-medium">
              ...
              {!isAdmin && (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 text-white hover:from-red-600 hover:to-pink-600 transition-all shadow-md text-xs font-medium"
                  title="Admin Login"
                >
                  <Lock size={14} />
                  Admin
        ...
        {notifications.length > 0 && (
          <div className="fixed top-6 right-6 z-50 space-y-2 max-w-md">
            {notifications.map(notif => (
              <div
                key={notif.id}
                className={`${currentTheme.card} rounded-xl shadow-lg border-2 p-4 animate-slide-in-right ${
        ...
        {activePage === 'admin' && isAdmin && (
          <AdminPanel
            ...
            newCleaningTask={newCleaningTask}
            setNewCleaningTask={setNewCleaningTask}
            handleAddCleaningTask={handleAddCleaningTask}
            handleDeleteCleaningTask={handleDeleteCleaningTask}
            calculateMonthlyStats={calculateMonthlyStats}
        ...
        <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3">
          ...
          {false && !showCheckInModal && checkedInEmployees.length < 3 && (
            <button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white px-5 py-3 rounded-full hover:from-blue-600 hover:to-purple-600 transition-all shadow-lg hover:shadow-xl hover:scale-105 text-sm font-medium"
              title="Start Shift"
            >
              <Clock size={20} />
          ...
          {!showAttentionSection && attentions.filter(a => !a.completed).length === 0 && (
            <button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-red-500 to-orange-500 text-white px-5 py-3 rounded-full hover:from-red-600 hover:to-orange-600 transition-all shadow-lg hover:shadow-xl hover:scale-105 text-sm font-medium"
        ...
        {showCheckInModal && (
          <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 backdrop-blur-sm">
            <div className={`${currentTheme.card} rounded-3xl shadow-2xl border-2 p-8 max-w-2xl w-full mx-4 relative`}>
              {/* Close Button */}
              <button
                onClick={() => setShowCheckInModal(false)}
                className="absolute top-4 right-4 p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
                title="Close"
              >
              ...
              <div className="text-center mb-6">
                ...
                <p className={`text-sm ${currentTheme.subtext}`}>
                  Shift Sekarang: <span className="font-bold">{detectShift() === 'pagi' ? '☀️ Pagi (09:00 - 17:00)' : '🌙 Malam (17:00 - 01:00)'}</span>
                </p>
                ...
                {employees.map(emp => (
                  <button
                    ...
                    className={`relative p-6 rounded-2xl border-2 transition-all ${
                      emp.checkedIn || isProcessing
                        ? 'bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-300 cursor-not-allowed opacity-70'
                        : 'bg-gradient-to-br from-blue-50 to-purple-50 border-blue-300 hover:border-blue-500 hover:shadow-xl hover:scale-105 cursor-pointer'
                    }`}
                  >
                    <div className="flex flex-col items-center gap-3">
                      <div className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-2xl shadow-lg ${
                        emp.checkedIn
                          ...
                          : 'bg-gradient-to-br from-blue-600 to-purple-600 text-white border-2 border-white/30'
                      }`}>
                      ...
                      <div className="text-center">
                        ...
                        {emp.checkedIn && (
                          <>
                            <p className="text-xs text-emerald-700 font-semibold mt-1">✓ Checked In</p>
                            <p className="text-xs text-emerald-600 font-medium">{emp.checkInTime}</p>
                            <span className={`inline-block mt-2 px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm ${
                              emp.shift === 'pagi'
                        ...
                        {!emp.checkedIn && (
                          <p className="text-sm text-slate-600 font-medium mt-1">Tap untuk check-in</p>
                        )}
                      </div>
                    </div>
                  </button>
              ...
              <div className="flex gap-3">
                {checkedInEmployees.length > 0 && (
                  <button
                    onClick={() => setShowCheckInModal(false)}
                    className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold hover:from-emerald-600 hover:to-teal-600 transition-all shadow-lg"
                  >
                    Mulai Bekerja ({checkedInEmployees.length} orang) →
                  </button>
                )}
                {checkedInEmployees.length === 0 && (
                  <p className="flex-1 text-center text-sm text-slate-500 py-3">
              ...
              {checkedInEmployees.length > 0 && checkedInEmployees.length < 3 && (
                <p className="text-center text-xs text-slate-400 mt-2">
                  💡 {3 - checkedInEmployees.length} orang lagi belum check-in (opsional)
                </p>
              )}
            </div>
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className={`${currentTheme.card} rounded-2xl shadow-2xl border-2 p-6 max-w-md w-full mx-4`}>
              ...
              <textarea
                ...
                className={`w-full px-4 py-3 rounded-lg border ${currentTheme.input} text-sm resize-none`}
                rows={4}
                autoFocus
              />
              ...
              <div className="flex gap-3 mt-4">
                <button
                  ...
                  className={`flex-1 px-4 py-2 rounded-lg ${currentTheme.badge} hover:opacity-80 transition-all text-sm font-medium`}
                >
                  Batal
                </button>
                <button
                  onClick={confirmPause}
                  className="flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-orange-500 to-red-500 text-white hover:from-orange-600 hover:to-red-600 transition-all text-sm font-medium shadow-md"
                >
                  Konfirmasi Pause
                </button>
              </div>
            ...
            <div className={`${currentTheme.card} rounded-2xl shadow-2xl border-2 p-6 max-w-md w-full mx-4`}>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-violet-500 to-purple-500 text-white transition-all text-sm font-medium shadow-md ${
                    isProcessing ? 'opacity-50 cursor-not-allowed' : 'hover:from-violet-600 hover:to-purple-600'
                  }`}
                >
            ...
            <div className={`${currentTheme.card} rounded-2xl shadow-2xl border-2 p-6 max-w-md w-full mx-4`}>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600 transition-all text-sm font-medium shadow-md"
                >
                  Konfirmasi Break
                </button>
              </div>
        ...
        <div className="fixed bottom-24 right-6 z-50 space-y-3 max-w-sm">
          {orderReminders.map(reminder => {
            ...
            const priorityConfig = {
              low: { color: 'slate', icon: '●', label: 'Low' },
              medium: { color: 'blue', icon: '●●', label: 'Medium' },
            ...
            return (
              <div
                key={reminder.id}
                className={`bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-2xl shadow-2xl border-2 border-blue-400 p-4 animate-bounce`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-2 text-xs">
                      ...
                      <span className={`bg-${priority.color}-600 px-2 py-0.5 rounded font-bold`}>
        ...
        <div className="fixed bottom-24 right-96 z-50 space-y-3 max-w-sm">
          {pauseReminders.map(reminder => {
            ...
            const iconConfig = {
              break: { icon: '☕', color: 'amber', label: 'Istirahat' },
              izin: { icon: '📋', color: 'violet', label: 'Izin' },
              task: { icon: '⏸️', color: 'orange', label: 'Pause Task' }
            };
            ...
            return (
              <div
                key={reminder.id}
                className={`bg-gradient-to-r from-${config.color}-500 to-${config.color}-600 text-white rounded-2xl shadow-2xl border-2 border-${config.color}-400 p-4 animate-bounce`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
      ...
      {showClearModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className={`${currentTheme.card} rounded-2xl shadow-2xl border-2 border-rose-500 p-6 max-w-md w-full mx-4`}>
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-rose-500 to-red-600 text-white hover:from-rose-600 hover:to-red-700 transition-all text-sm font-medium shadow-md"
              >
                🗑️ Hapus Semua Data
              </button>
            </div>
      ...
      {showAdminLogin && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className={`${currentTheme.card} rounded-2xl shadow-2xl border-2 border-red-500 p-6 max-w-md w-full mx-4`}>
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-pink-500 text-white hover:from-red-600 hover:to-pink-600 transition-all text-sm font-medium shadow-md ${isLoggingIn ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
      ...
      {editingCalendar && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className={`${currentTheme.card} rounded-2xl shadow-2xl border-2 border-emerald-500 p-6 max-w-md w-full mx-4`}>
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 text-white hover:from-emerald-600 hover:to-teal-600 transition-all text-sm font-medium shadow-md"
              >
                💾 Simpan
              </button>
            </div>
