...
import { distributeTasks } from '../utils/taskDistribution';
...
export const processCheckIn = (employee, currentShift, allEmployees) => {
  ...
  const employeeIndex = allEmployees.findIndex(e => e.id === employee.id);
  const totalEmployees = allEmployees.length;
  const distributedTasks = distributeTasks(currentShift, totalEmployees);
  const employeeTasks = distributedTasks[employeeIndex] || [];
  ...
  // Create task objects
  const tasks = employee.checkedIn ? employee.cleaningTasks : employeeTasks.map((task, i) => ({
    id: Date.now() + employee.id * 1000 + i,
    task: task,
    completed: false,
    priority: 'normal',
    progress: 0,
...
export const processCheckOut = (employee) => {
  ...
  // Save all tasks to history
  const allCleaningTasks = (employee.cleaningTasks || []).map(t => ({
    ...t,
  ...
  return {
    ...
    cleaningTasks: [],
    workTasks: [],
    completedTasksHistory: [...(employee.completedTasksHistory || []), ...newHistory]
  };
};
