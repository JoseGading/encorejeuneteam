      ...
      {active.length > 0 && (
        <div className={`${currentTheme.card} rounded-2xl shadow-md border-2 p-5`}>
          ...
          <div className="space-y-2.5">
            {active.map(a => (
              <div key={a.id} className={`${currentTheme.badge} border border-white/10 rounded-xl p-3`}>
              ...
              <div key={`break-${emp.id}`} className="mb-4">
                ...
                <div className="space-y-2">
                  {filteredBreaks.map((brk, idx) => (
                    <div key={`break-${emp.id}-${idx}-${brk.date}`} className={`${currentTheme.badge} rounded-lg p-3 border ${brk.isLate ? 'border-blue-500/30' : 'border-white/10'}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
              ...
              <div key={`izin-${emp.id}`} className="mb-4">
                ...
                <div className="space-y-2">
                  {filteredIzin.map((izin, idx) => (
                    <div key={`izin-${emp.id}-${idx}-${izin.date}`} className={`${currentTheme.badge} rounded-lg p-3 border border-white/10`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                  ...
                  return (
                    <div key={day} className="relative">
                      {hasData ? (
                        ...
                        <div 
                          className={`calendar-cell ${currentTheme.badge} rounded-lg p-2.5 text-center text-xs font-medium border border-dashed border-white/10 ${currentTheme.text}`}
