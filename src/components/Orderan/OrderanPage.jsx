    ...
    <div className="space-y-6">
      ...
      <div className={`${currentTheme.card} rounded-2xl ${currentTheme.shadow} border-2 ${currentTheme.borderColor} p-4 sm:p-6 mb-6`}>
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 lg:pb-0">
            {['all', 'pending', 'process', 'completed'].map((status) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl border-2 font-bold text-xs sm:text-sm transition-all whitespace-nowrap ${orderFilter === status
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 lg:pb-0">
            {['all', 'wa', 'tele', 'shopee_media', 'shopee_acc'].map((platform) => (
              <button
                key={platform}
                onClick={() => setOrderPlatformFilter(platform)}
                className={`px-4 py-2 rounded-xl border-2 font-bold text-xs sm:text-sm transition-all whitespace-nowrap ${orderPlatformFilter === platform
                  ? `${currentTheme.accent} text-white border-transparent shadow-md`
                  : `${currentTheme.badge} ${currentTheme.borderColor} border-opacity-30`
                  }`}
              >
                {platform === 'all' ? 'All Platform' : getPlatformBadge(platform).label}
              </button>
            ))}
          </div>
          <button
            ...
            className={`px-4 py-2 rounded-lg ${currentTheme.badge} hover:bg-white/5 transition-all font-medium text-xs sm:text-sm whitespace-nowrap`}
          >
            Reset Filter
          </button>
        </div>
                          ...
                          {order.notes && order.notes.length > 0 ? (
                            <div className="space-y-2 mb-3">
                              {order.notes.map(note => (
                                <div key={note.id} className={`${currentTheme.badge} rounded-lg p-3 min-w-0`}>
                                  <p className={`text-sm ${currentTheme.text} whitespace-pre-wrap break-words`}>{formatNoteText(note.text)}</p>
