          ...
          <div className="flex items-center gap-3 relative z-30">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2 rounded-lg ${currentTheme.badge} hover:opacity-70 transition-all cursor-pointer relative z-30`}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 relative z-30">
            <button
              ...
              className={`p-2 rounded-lg ${currentTheme.badge} hover:opacity-70 transition-all disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer relative z-30`}
          ...
          return (
            <div 
              ...
              className={`${currentTheme.card} rounded-xl shadow-md border-2 p-4 transition-all hover:shadow-lg ${
                isDoubleShift ? 'border-purple-400 bg-purple-50' : ''
              }`}
            >
              {/* Date Header */}
              <div className="flex items-center justify-between mb-3 pb-3 border-b-2 border-slate-200">
                <div>
                  <div className={`text-2xl font-bold ${isDoubleShift ? 'text-purple-900' : currentTheme.text}`}>{row.day}</div>
                  <div className={`text-xs ${row.dayName === 'Minggu' ? 'text-red-500 font-semibold' : isDoubleShift ? 'text-purple-700 font-semibold' : currentTheme.subtext}`}>
                    {row.dayName}
                  </div>
                </div>
                <div className={`text-xs ${isDoubleShift ? 'text-purple-600 font-medium' : 'text-slate-400'}`}>{row.date}</div>
              </div>
